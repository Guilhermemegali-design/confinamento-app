...
const percentualComSinal = (n) => `${n > 0 ? "+" : n < 0 ? "-" : ""}${Math.abs(n).toLocaleString("pt-BR",
  Math.abs(n) > 0 && Math.abs(n) < 0.01 ? { maximumSignificantDigits: 2 } : { maximumFractionDigits: 2 })}%`;
...
export async function criarPdfRelatorioTrato(relatorio, { clienteNome = "Confinamento", destinatario = "", geradoEm = new Date() } = {}) {
  ...
  function cabecalhoAcumulado(primeiraColuna) {
    ...
    texto("ERRO %", M + 527, y + 20, 10, "bold", "#FFFFFF", { align: "right" });
  ...
  function tabelaAcumulada({ titulo, contexto, linhasTabela, resumo, maximo, primeiraColuna = "INGREDIENTE" }) {
    ...
    function desenharResultado(r, altura) {
      ...
      const erroPercentual = resultado.diferenca != null && r.previsto > 0
        ? resultado.diferenca / r.previsto * 100 : null;
      const erroTexto = erroPercentual != null ? percentualComSinal(erroPercentual)
        : resultado.diferenca == null ? "Sem dados" : "Sem base";
      valorCelula(erroTexto, M + 527, centro + 5, 70, erroPercentual == null ? 10 : 15, cor);
      if (resultado.quantidade > 0) {
        doc.setFillColor(cor);
        doc.rect(M + 548, centro - 7, resultado.quantidade / maximo * 62, 14, "F");
      }
      ...
      valorCelula(valor, W - M - 12, centro + 17, 145, resultado.quantidade > 0 ? 20 : 11, cor);
    }
    abrir();
