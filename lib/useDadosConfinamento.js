...
export function useDadosConfinamento(consultorId) {
  ...
  const carregarTudo = useCallback(async () => {
    ...
    const { data: dadosLeiturasCocho } = await supabase.from("leituras_cocho").select("*").eq("consultor_id", consultorId);
    setLeiturasCocho(dadosLeiturasCocho || []);

    const { data: dadosClientesUsuarios } = await supabase
      .from("clientes_usuarios")
      .select("*")
      .eq("consultor_id", consultorId);
    setClientesUsuarios(dadosClientesUsuarios || []);
