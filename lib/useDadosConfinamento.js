...
export function useDadosConfinamento(consultorId) {
  ...
  async function adicionarConsumo(loteId, dados) {
    const { data, error } = await supabase
      .from("consumos_lote")
      // Um lote só pode ter um consumo consolidado por dia. Se o usuário
      // lançar novamente a mesma data, atualiza o registro diário existente.
      .upsert(
        { ...dados, lote_id: loteId, consultor_id: consultorId },
    ...
    if (error) throw error;
    setConsumos((cs) => {
      const existe = cs.some((c) => c.lote_id === loteId && c.data === data.data);
      return existe
        ? cs.map((c) => (c.lote_id === loteId && c.data === data.data ? data : c))
        : [...cs, data];
    });
    return data;
  }
