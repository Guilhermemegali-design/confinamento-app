...
export function calcularIndicadoresLote(lote, pesagens = [], consumos = [], saidas = [], entradas = []) {
  const hoje = hojeISO();
  ...
  const consumosMSHistoricos = consumosOrdenados
    .map((consumo) => {
      ...
      return { consumoMSCabeca, percentualPV: pesoNaData > 0 ? (consumoMSCabeca / pesoNaData) * 100 : null };
    })
    .filter((registro) => registro != null);
  const consumoMSMedio = consumosMSHistoricos.length
    ? consumosMSHistoricos.reduce((soma, r) => soma + r.consumoMSCabeca, 0) / consumosMSHistoricos.length
    : null;
  const percentuaisMSHistoricos = consumosMSHistoricos
    .map((r) => r.percentualPV)
    .filter((percentual) => percentual != null);
  const consumoMSPercentualPVMedio = percentuaisMSHistoricos.length
    ? percentuaisMSHistoricos.reduce((soma, percentual) => soma + percentual, 0) / percentuaisMSHistoricos.length
    : null;
  ...
  const { custoAcumuladoAnimal, custoMedioDiarioAnimal } = calcularCustoAcumulado(lote, consumos, saidas, entradas);
...
export function calcularCustoAcumulado(lote, consumos = [], saidas = [], entradas = []) {
  ...
  // Animais trocados de lote chegam com um custo já investido neles (compra
  // rateada + alimentação acumulada no lote de origem, ver FormTransferencia)
  // gravado em entradas_lote.custo_acumulado_herdado. Isso entra aqui como
  // uma média ponderada pelas cabeças de cada entrada, dividida pelas
  // cabeças atuais — pra não desaparecer do custo do lote de destino.
  const cabecasAtuais = calcularCabecasNaData(lote, saidas, dataFim, entradas);
  const custoHerdadoTotal = entradas.reduce(
    (s, e) => s + (e.custo_acumulado_herdado != null ? Number(e.custo_acumulado_herdado) * Number(e.num_cabecas || 0) : 0),
    0
  );
  const custoHerdadoPorCabeca = custoHerdadoTotal > 0 && cabecasAtuais > 0 ? custoHerdadoTotal / cabecasAtuais : 0;
  ...
  if (custosPorData.size === 0) {
    return custoHerdadoPorCabeca > 0
      ? { custoAcumuladoAnimal: custoHerdadoPorCabeca, custoMedioDiarioAnimal: null }
      : { custoAcumuladoAnimal: null, custoMedioDiarioAnimal: null };
  }
  ...
  return { custoAcumuladoAnimal: acumulado + custoHerdadoPorCabeca, custoMedioDiarioAnimal };
}
...
export function calcularFechamentoCusto(lote, indicadores, saidasLancadas = []) {
  ...
  // Mortes (tipo "morte") não geram receita nem arroba vendida — são
  // excluídas daqui, mas continuam contando nos animal-dias (custo de
  // alimentação) porque o animal comeu até morrer.
  const saidasVendidas = saidas.filter((s) => s.tipo !== "morte");
  ...
  const saidasComCustoOperacional = saidasVendidas.filter((s) => s.custo_operacional != null && s.data != null);
  const custoOperacionalTotal =
    saidasComCustoOperacional.length > 0
      ? saidasComCustoOperacional.reduce((soma, s) => {
          ...
          return soma + Number(s.custo_operacional) * dias * Number(s.num_cabecas || 0);
        }, 0)
      : null;
  ...
  const custoProducaoTotal =
    custoAlimentarTotal != null ? custoAlimentarTotal + (custoOperacionalTotal || 0) : null;
