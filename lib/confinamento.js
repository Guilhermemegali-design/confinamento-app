...
// pesagensPorLote / consumosPorLote: objetos { [lote_id]: registros[] }
export function calcularPainelConfinamento(lotes, pesagensPorLote = {}, consumosPorLote = {}) {
  ...
  const somaCabecasComCustoAcumuladoAtivos = comCustoAcumuladoAtivos.reduce((s, i) => s + Number(i.lote.num_cabecas || 0), 0);
  ...
  const somaCabecasComCustoMedioDiarioAtivos = comCustoMedioDiarioAtivos.reduce((s, i) => s + Number(i.lote.num_cabecas || 0), 0);
...
// Acha o lançamento de consumo a usar como referência para a leitura de
// cocho de hoje: o mais recente até ontem (inclusive). Se o cliente pulou
// um dia de lançamento, ainda assim usa o último disponível — melhor do
// que travar a leitura esperando um lançamento exato de ontem.
export function obterConsumoReferenciaCocho(consumos = []) {
  const ontem = new Date();
  ontem.setDate(ontem.getDate() - 1);
  const ontemISO = ontem.toISOString().slice(0, 10);
  const anteriores = consumos
    .filter((c) => c.data <= ontemISO)
