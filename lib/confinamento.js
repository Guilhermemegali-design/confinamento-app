...
export function calcularIndicadoresLote(lote, pesagens = [], consumos = [], saidas = []) {
  ...
  const consumoMSPercentualPV =
    consumoMS != null && pesoEstimadoUltimoConsumo > 0
      ...
      : null;
  const percentuaisMSHistoricos = consumosOrdenados
    .map((consumo) => {
      const cabecasNaData = calcularCabecasNaData(lote, saidas, consumo.data);
      ...
      return pesoNaData > 0 ? (consumoMSCabeca / pesoNaData) * 100 : null;
    })
  ...
  return {
    ...
    consumoMSPercentualPV, consumoMSPercentualPVMedio,
...
export function calcularFechamentoCusto(lote, indicadores, saidasLancadas = []) {
  ...
  const saidasComPesoVivo = saidas.filter((s) => s.peso_saida_vivo != null);
  ...
  const arrobasProduzidasVivo =
    lote.peso_entrada != null && cabecas > 0 && saidasComPesoVivo.length > 0
      ? (pesoVivoSaidaTotal - Number(lote.peso_entrada) * cabecas) / 15
      : null;
  ...
  // Custo da arroba obtida após o abate: considera todo o capital investido
  // (animal + alimentação + operacional) e divide pelas arrobas de carcaça
  // efetivamente vendidas, já aplicando o rendimento de abate.
  const custoArrobaProduzidaRendimento =
    custoTotalGeral != null && arrobasVendidasTotal > 0 ? custoTotalGeral / arrobasVendidasTotal : null;
  const gmc =
